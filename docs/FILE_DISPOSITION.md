...
| `docs/INTERCHANGE_CONTRACT.md`, `PWA_ASSURANCE_CONTRACT.md`, `HEALTHOS_CONTRACT.md`, `LIFEOS_CONTRACT.md`, `STRENGTH_CONTRACT.md`, `REFLECTION_CONTRACT.md`, `REPO_ZIP_POLICY.md`, `future-app-intake.md` | 8 | KEEP as bounded A3 contracts; revise only with implementation, tests, and decisions. |
...
| `tests/*.mjs`, `tests/*.spec.mjs`, `tests/fixtures/*` | 26 | KEEP/EXTEND; R0–R3D isolation, scoring, PWA, OmniCore, LifeOS, Strength/Reflection controllers, icon, artifact, provenance, responsive, and automated accessibility gates are present. |
...
| `apps/noodle-nudge/index.html`, `service-worker.js`, `scoring.js`, `reflection-adapter.js`, `reflection/*` | 7 | KEEP; R0 removed executable scoring; R3A versioned ten-definition/42-rule pure Reflection seams; R3D reduced the HTML to a 65-line shell while preserving the old scoring URL and independent runtime. |
| `apps/noodle-nudge/app.js`, `styles.css`, `controller/*.js` | 10 | KEEP/HARDEN as the 119-line composition root, external app styling, and Noodle-owned state, storage, content, session, settings/recovery, safe view/chart, and lifecycle/DOM seams; retain only the nine evidenced compatibility bindings. |
...
Counts reflect the 300-file R3D evidence-close layout. Before any later deletion, prove no route, manifest, worker shell, import, migration, test, license, provenance record, or user recovery path depends on the target.
