...
`/goal Complete bounded LocalFirstApps R3B CommonGround LifeOS Strength foundation after verified R3A. Reconcile live authority/state first. Preserve Flexx Files route, v3 scoped flexx_ localStorage keys and shapes, drafts, sessions, preferences, JSON backup/restore, reset boundary, manifest, worker/cache scope, offline behavior, UI, calculations, and HealthOS/Noodle independence. Inventory every persisted key/schema and global-controller dependency. Extract only already-proven pure plate loading, progression, deload/readiness, validation, and recovery functions into versioned Flexx-owned Strength modules with an app-owned adapter; make the existing UI consume each extracted seam only after exact characterization parity. Add deterministic fixtures for all calculations, legacy/current backups, drafts, malformed/quota/storage failures, and unrelated localStorage/cache survival. Produce an explicit exact preview-only future LifeOS mapping fixture with mutation forbidden; do not migrate, dual-write, rename, delete, or read Flexx data from HealthOS/LifeOS. Do not approve training/health content, activate later HealthOS modules, change Noodle Reflection, rewrite TS-Dash, begin WorkOS, create shared storage/sync/global bus/universal worker/backend/accounts/AI, retire routes, or publish. Require code-only rollback, complete offline shells, responsive/file/accessibility parity, and unchanged outputs/formats. Update state/decisions/contracts/evidence/handoff only to observed facts. Run npm run ci:candidate, npm audit --audit-level=moderate, git diff --check, and clean tracked-status gates; make bounded local commits only.`

Do not start R3B until R3A is reviewed. Later packets must be regenerated from current state.
