...
`/goal Complete bounded LocalFirstApps R3C Flexx Strength controller modularization after verified R3B. Reconcile live authority, Git state, contracts, and tests first. Preserve the Flexx route, HTML-visible workflow, calculations, timing, translations, keyboard/pointer/touch behavior, responsive/file/offline/PWA behavior, v3 flexx_ keys and record shapes, drafts, sessions, preferences, legacy/current JSON backup/restore, reset/cache/worker boundaries, all outputs, and HealthOS/Noodle independence. Characterize every state transition, rendered state, modal, timer path, and the exact 20 window handlers before changing ownership. Incrementally extract app-owned state/selectors, commands, timer coordination, and safe view/render modules from js/app.js; keep a thin compatibility binding for existing HTML handlers and consume only the R3B Strength adapter/storage facade. Replace string/event coupling only where parity fixtures prove exact behavior; preserve focus, announcements, error visibility, and reduced motion. Add deterministic unit and browser faults for resume/draft, progression/deload, timers, swaps, pagination/chart/protocol, import quota/malformed data, reset cancellation/failure, multi-tab/storage events, and unrelated storage/cache survival. Do not migrate/dual-write/rename/delete data, grant LifeOS foreign-store access, alter training rules/content, activate later HealthOS modules, change Noodle, rewrite TS-Dash, begin WorkOS, add shared runtime state/sync/bus/worker/backend/accounts/AI, retire routes, or publish. Rollback must be code-only. Update authority/state/decision/contract/evidence/handoff files only to observed facts. Run focused parity tests, npm run ci:candidate, npm audit --audit-level=moderate, git diff --check, and clean tracked-status gates; make bounded local commits only.`

Do not start R3C until R3B is reviewed. Later packets must be regenerated from current state.
