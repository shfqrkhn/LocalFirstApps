...
- Standalone redirect folders or URLs for retired apps. The sole temporary exception is the unlisted `apps/ledgersuite` migration redirect in `v0.2.0`; `.gitattributes` must exclude it from generated repository archives, and the next genuine feature release must remove it.
